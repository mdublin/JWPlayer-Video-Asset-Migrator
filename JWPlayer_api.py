...
def create(**kwargs):
    ...
    try:
        response = jwplatform_client.videos.create(title=str(kwargs["title"]), description=str(kwargs["description"]), tags=str(kwargs["tags"]), sourceurl=str(kwargs["sourceurl"]), sourcetype=kwargs["sourcetype"], sourceformat=kwargs["sourceformat"])
        #print(response)
        print("")
